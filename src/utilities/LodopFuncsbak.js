...
function getLodop(oOBJECT, oEMBED) {
  ...
  var strCLodopInstallA = 'Web打印服务CLodop未安装启动，请先下载安装'
  // var strCLodopInstallB = "<br>（若此前已安装过，可<a href='CLodop.protocol:setup' target='_self'>点这里直接再次启动</a>）";
  var strCLodopInstallB = "（若此前已安装过，可<a href='CLodop.protocol:setup' target='_self'>点这里直接再次启动</a>）"
  // var strCLodopUpdate = "<br><font color='#FF00FF'>Web打印服务CLodop需升级!点击这里<a href='CLodop_Setup_for_Win32NT.exe' target='_self'>执行升级</a>";
  var strCLodopUpdate = "Web打印服务CLodop需升级!点击这里<a href='CLodop_Setup_for_Win32NT.exe' target='_self'>执行升级</a>"
  ...
  try {
    ...
    if (needCLodop() || isLinuxX86 || isLinuxARM) {
      ...
      if (!LODOP) {
        ...
        if (isLinuxX86 && LODOP.CVERSION < '7.0.4.3') { strAlertMessage = strLodop7Update_X86 } else if (isLinuxARM && LODOP.CVERSION < '7.0.4.3') { strAlertMessage = strLodop7Update_ARM } else if (CLODOP.CVERSION < '6.5.7.1') { strAlertMessage = strCLodopUpdate }

        if (strAlertMessage) { alert(strAlertMessage + strInstallOK) }
      }
    } else {
      ...
      if (LODOP.VERSION < '6.2.2.6') {
        ...
        alert((isWinIE64 ? strLodop64Update : strLodopUpdate) + strInstallOK)
      }
    }
